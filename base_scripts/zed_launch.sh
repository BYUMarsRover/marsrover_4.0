...
if [ ! $(ssh $ROVER_USERNAME@$ROVER_IP_ADDRESS "docker ps" | grep -q "zed-ct") ]; then
    ...
    if [ $(ssh $ROVER_USERNAME@$ROVER_IP_ADDRESS "uname -m") == "aarch64" ]; then
        ...
        ssh -t $ROVER_USERNAME@$ROVER_IP_ADDRESS "cd ~/marsrover_4.0/zed_ws/docker/arm_orin && docker compose up -d"
    else
        printInfo "Rover is running on AMD architecture, using the amd_desktop image..."
        ssh -t $ROVER_USERNAME@$ROVER_IP_ADDRESS "cd ~/marsrover_4.0/zed_ws/docker/amd_desktop && docker compose up -d"
    fi
    ssh -t $ROVER_USERNAME@$ROVER_IP_ADDRESS "cd ~/marsrover_4.0/zed_ws/docker && docker compose up -d"
fi
...
cd $script_dir && envsubst < tmuxp/autonomy/zed_launch.yaml > tmuxp/tmp/zed_launch.yaml
scp tmuxp/tmp/zed_launch.yaml $ROVER_USERNAME@$ROVER_IP_ADDRESS:~/marsrover_4.0/base_scripts/tmuxp/tmp/
